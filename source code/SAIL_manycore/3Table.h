...
class CFib
{
	...
	public:
		...
		//OH algorithm
		void OHConstruction(FibTrie* pTrie, int level, char *prefix);
		void Buildbitmap24(FibTrie* pTrie,int level, char *prefix);
		...
		void HashTable32_Construction(FibTrie* pTrie,unsigned int iVal,int iBitLen);
		int HashTableConstructor();
		int HashTableInsert( IPNode *InsertIPNode);
};
...
class ThreeTable{
	...
	public:
...
#else 
		double lookupbyhth (const char* trace_path, int size, const char * ribfileName) {
			...
			double returnvalue = lookupKernelTable (table_16.data, table_24.data,  hash_table32.data1,hash_table32.data2, result, traffic, size);
			// check result
		...
		double runLookupV4 (const char* trace_path, int size, const char * ribfileName) {
			...
			//fin>>traceNum;
			printf("open file %s successfully!", trace_path);
		...
		double lookupKernelTable (element_16* ele16, element_24* ele24, IPNode * hashtable_1, IPNodeList *hashtable_2, unsigned int* result, unsigned int* traffic, int size) 
		{
			...
			int eachTime =  (end.tv_sec - begin.tv_sec) * 1e6 + end.tv_usec - begin.tv_usec;
			printf("Lookup time=%lf\n", 1.0*eachTime/1e6);
			return 1.0*eachTime/1e6;
		}

		double lookupKernelTableByHth(element_16* ele16, element_24* ele24, IPNode * hashtable_1, IPNodeList *hashtable_2, unsigned int* result, unsigned int* traffic, int size) 
		{
			register unsigned char LPMPort=0;
			register unsigned int taffic_Integer=0;
